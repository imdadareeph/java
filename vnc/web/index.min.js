	...
	function serializeCombo(objId){var inputObj=byId(objId);var listObj=byId(objId+"_list");if(!inputObj){return null}if((!listObj)||(listObj.options.length==0)){return null}var data=new Array();if(inputObj.value!=""){data.push(inputObj.value)}for(var n=0;n<listObj.options.length;n++){if(listObj.options[n].value!=inputObj.value){data.push(listObj.options[n].value)}}if(data.length>15){data.splice(15,data.length-14)}return data}function unserializeCombo(objId,data,defaultValues){var inputObj=byId(objId);var listObj=byId(objId+"_list");if(!(inputObj&&listObj)){return}if(!data){data=defaultValues}for(idx in data){listObj.options[listObj.options.length]=new Option(data[idx],data[idx])}if(listObj.options.length>0){inputObj.value=listObj.options[0].value}}
		...
		function showPane(){
			...
			$("ul.tabs li").click(function(){
				...
				$(".tab_content").hide();
				var activeTab=$(this).find("a").attr("href");$(activeTab).fadeIn();return false})}function filterDisplay(screenSharedSelected){if((lastDisplaySelected!=null)&&(screenSharedSelected==lastDisplaySelected)){return}var bpp=byId("bpp");var bppauxVNC=byId("bppauxVNC");var bppauxRDP=byId("bppauxRDP");var res=byId("resolution");var resauxVNC=byId("resauxVNC");var resauxRDP=byId("resauxRDP");var imgq=byId("imgquality");var imgqauxVNC=byId("imgqauxVNC");var imgqauxRDP=byId("imgqauxRDP");if(screenSharedSelected){moveOptions(bpp,bppauxRDP);moveOptions(bppauxVNC,bpp);moveOptions(res,resauxRDP);moveOptions(resauxVNC,res);moveOptions(imgq,imgqauxRDP);moveOptions(imgqauxVNC,imgq)}else{moveOptions(bpp,bppauxVNC);moveOptions(bppauxRDP,bpp);moveOptions(res,resauxVNC);moveOptions(resauxRDP,res);moveOptions(imgq,imgqauxVNC);moveOptions(imgqauxRDP,imgq)}lastDisplaySelected=screenSharedSelected}function filterAdvanced(screenSharedSelected){var rows=$("#tabAdvanced tr");var displayStat=(screenSharedSelected)?"none":"";for(var r=0;r<2;r++){rows[r].style.display=displayStat}}function setDefaults(){var sel;if(si.features.vnc){sel="VNC"}else{if(si.features.rdp){sel="RDP"}else{if(si.features.rdp){sel="FT"}else{sel="VNC"}}}document.getElementById("connectionType"+sel).click();document.getElementById("newwindow").checked=true;document.getElementById("resauxVNC").value="remote";document.getElementById("bppauxVNC").value="16";document.getElementById("imgqauxVNC").value="1";document.getElementById("controlMode").checked=true;byId("wscompression").checked=true;byId("relativeTouch").checked=true;document.getElementById("resauxRDP").value="fittobrowser";document.getElementById("bppauxRDP").value="16";document.getElementById("imgqauxRDP").value="1";byId("vedesktopbackground").checked=false;byId("vemnuwndanimation").checked=false;byId("vevisualstyles").checked=true;byId("vefontsmoothing").checked=false;byId("veshowwndcontent").checked=false;byId("vedesktopcomposition").checked=false;byId("unicodekeyb").checked=true;byId("console").checked=false;byId("wscompression").checked=true;byId("relativeTouch").checked=true}function moveOptions(selectSource,selectTarget){var idx=selectSource.selectedIndex;while(selectSource.options.length>0){selectTarget.appendChild(selectSource.options[0])}if(idx>-1){selectTarget.options[idx].selected=true}}$("#options").click(showOptions);
	$("#connectionTypeVNC").click(function(){$("#optExperience").hide();filterDisplay(true);filterAdvanced(true);$("#optDisplay,#optControl,#optAdvanced").show()});
	$("#connectionTypeRDP").click(function(){$("#optControl").hide();filterDisplay(false);filterAdvanced(false);$("#optDisplay,#optExperience,#optAdvanced").show()});
	$("#connectionTypeFT").click(function(){$("#optDisplay,#optControl,#optExperience,#optAdvanced").hide()});
	$("#connectBtn").click(function(){connect()});
	$("#loginCancel").click(function(){$.unblockUI()});
...
canConfig=HTML5Storage.supportsHTML5Storage();
if(canConfig){readSettings()}
if(window.location.pathname!="/"){
	document.body.style.visibility="hidden";
	$("#connectionTypeVNC").click();
	connect(true)
}
		else{
			window.name="";
			$("#connectionTypeVNC").attr("disabled",!si.features.vnc);
			$("#connectionTypeRDP").attr("disabled",!si.features.rdp);
			$("#connectionTypeFT").attr("disabled",!si.features.ft);
			if(authenticated){throwPing()}showPane()
		}
	}
);

//remove this -- self created imdad
